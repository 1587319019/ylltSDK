...
module.exports = {
		...
		pool.getConnection((err, connection) => {
			...
			connection.query(sqlMap.user.queryUsername, [ postData.username ], (err, result) => {
				if (result.length === 0) {
					...
					if (result[0].password !== postData.password.substring(0, postData.password.length - 2)) {
						res.json({
							status: false,
							msg: '密码错误'
						});
	...
	getArticle(req, res, next) {
		pool.getConnection((err, connection) => {
			...
			if (postData.username !== undefined && postData.pageNum !== undefined) {
				connection.query(
					sqlMap.article.queryAllBysu,
					[ postData.state, postData.username, start, end ],
					(err, result) => {
						res.json(result);
					}
				);
	...
	addViewOrStart(req, res, next) {
		...
		pool.getConnection((err, connection) => {
			let postData = req.body;
			connection.query(
				sqlMap.article.updateViewCount,
				[ postData.view, postData.count, postData.id ],
				(err, result) => {
					//res.send('ok');
					res.json(result);
				}
	...
	updArticle(req, res, next) {
		pool.getConnection((err, connection) => {
			let postData = req.body;
			connection.query(
				sqlMap.article.updAllById,
				[ postData.state, postData.title, postData.tags, postData.content, postData.id ],
				(err, result) => {
					if (err !== null) {
						res.json({
							status: false,
